...
--readF :: String -> Rational
readF s = ipart + fpart
  where
    [i,f] = wordsBy (=='.') s
    ipart = readInteger i % 1
    fpart = fsub deci f
    fsub w ('(':xs) = let cpart = init xs in w / deci * (readInteger cpart % (10^(length cpart) - 1))
    fsub w (x:xs) = let d = (toInteger . digitToInt) x in w * (d % 1) + fsub (w*deci) xs
    fsub w [] = 0
    deci = 1 % 10

showR r = show (numerator r) ++ "/" ++ show (denominator r)

main = getLine >>= putStrLn . showR . readF
