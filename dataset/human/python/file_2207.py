...
void main()
{
    ...
    auto qList = scanElem!(long,long)(D)
        .zip(iota(D))
        .map!(a=>Q(a[1]*100+100, a[0][0], a[0][1]))
        .array;
    ...
    foreach(bits;0..1L<<D)
    {
        ...
        foreach(i;0..D){
            if((bits&(1L<<i))!=0)continue;
            auto q = qList[i];
            auto c = max(0,(G-score+q.n-1)/q.n);
            if(c>q.c)continue;
            addCount = min(c, addCount);
        }
        res = min(addCount+count, res);
    }
    writeln(res);
}
...
void end(T)(T v)
if(isIntegral!T||isSomeString!T||isSomeChar!T||is(typeof(T)==MInt))
{
