...
main = do
    [n,m] <- sLineToIntL
    fts <- mLinesToTupleV m
    first <- VUM.replicate (2*10^5+1) False :: IO (VUM.IOVector Bool)
    second <- VUM.replicate (2*10^5+1) False :: IO (VUM.IOVector Bool)
    VU.forM_ fts $ \(f,t) -> do
        when (f == 1) $ VUM.write first t True
        when (t == n) $ VUM.write second f True
    
    fs <- VU.unsafeFreeze first 
    ss <- VU.unsafeFreeze second
    putStrLn $ 
        if (VU.or $ VU.zipWith (&&) fs ss)
            then "POSSIBLE"
            else "IMPOSSIBLE"
--------------------------------------------------------------------------
