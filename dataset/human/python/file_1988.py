...
step acc x
    | x == 0 = acc
    | otherwise = (x+1) * acc `mod` 1000000007

main = do
    n <- int
    xs <- VU.map (\c -> ord c - ord 'a') . VU.fromList <$> str
    let
        base = VU.replicate 26 0 :: VU.Vector Int
        add = VU.replicate n 1 :: VU.Vector Int
        bucket = VU.accumulate_ (+) base xs add
        res = VU.foldl' step 1 bucket
    print $ res - 1
--------------------------------------------------------------------------
