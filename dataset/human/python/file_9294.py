...
--------------------------------------------------------------------------
count table xs curr cnt = do
    let dest = xs VU.! curr
    r <- VUM.read table curr
    if (r /= 0)
        then do
            return (cnt,r)
        else do
            VUM.write table curr cnt
            count table xs dest (cnt+1)

simulate xs start rest
    | rest == 0 = start
    | otherwise = simulate xs x (rest-1)
    where
        x = xs VU.! start

main = do
    [n,k] <- sLineToIntL
    xs <- VU.map m1 <$> sLineToIntV n
    table <- VUM.replicate n 0 :: IO (VUM.IOVector Int)
    (nextC,prevC) <- count table xs 0 1
    if k < nextC
        then do
            print $ (simulate xs 0 k) + 1
        else do
            let
                cycle = nextC - prevC
                rest = k - nextC + 1
