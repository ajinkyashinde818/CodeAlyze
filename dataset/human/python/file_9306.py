...
void main(){
    auto N = scanElem;
    auto K = scanElem;
    auto list = scanLineArray.map!"a-1".array;

    auto table = new long[N];
    table[] = -1;
    table[0] = 0;
    long c;
    debugPrint!list;
    while(table[list[c]]==-1){
        if(K==0){
            end(c+1);
        }
        K--;
        debugPrint!c;
        table[list[c]] = table[c]+1;
        c = list[c];
    }
    debugPrint!table;
    K %= (table[c] - table[list[c]])+1;
    debugPrint!(c, K, ()=>table[c], ()=>table[list[c]]);
    while(true){
        if(K==0){
            end(c+1);
        }
