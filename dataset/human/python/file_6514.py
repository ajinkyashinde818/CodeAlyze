...
func readWords<T>(_ f: (Substring) -> T) -> [T] {
...
func main() {
    ...
    var origin = true
    for _ in 1...q {
        let query = readWords { String($0) }
        if query[0] == "1" {
            origin = !origin
        } else {
            if query[1] == "1" {
                if origin {
                    deque.pushFront(query[2])
                } else {
                    deque.pushBack(query[2])
                }
            } else {
                if origin {
                    deque.pushBack(query[2])
    ...
    var ans = [String]()
    while let c = origin ? deque.popFront() : deque.popBack() {
        ans.append(c)
    }
