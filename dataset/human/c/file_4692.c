...
int Height(int c){
  ...
  if(tree[c].left==-1) return Height(tree[c].right)+1;
  if(tree[c].right==-1) return Height(tree[c].left)+1;
  if(Height(tree[c].left)<Height(tree[c].right))
    return Height(tree[c].right)+1;
  else return Height(tree[c].left)+1;
  return d;
}
...
void printTree(){
  ...
  for(i=0;i<n;i++){
    dep=Depth(i);
    h=Height(i);
    printf("node %d: parent = %d, sibling = %d, degree = %d, depth = %d, height = %d, ",i,tree[i].parent,Sibling(i),Degree(i),dep,h);
