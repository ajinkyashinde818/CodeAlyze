...
typedef struct{
  int par, left_child, right_sibling, depth, status;
}Node;
...
main(){
  ...
  for(i=0;i<n;i++){
    ...
    scanf("%d", &idx);
    scanf("%d", &nchild[idx]);
    if(nchild[idx] == 0) tree[idx].status = 1;
    else tree[idx].status = 2;
  ...
  calc_depth(r, 0);
  for(i=0;i<n;i++){
    printf("node %d: parent = %d, depth = %d, ", i, tree[i].par, tree[i].depth);
    if(tree[i].status == 0) printf("%s, [", "root");
    else if(tree[i].status == 1) printf("%s, [", "leaf");
    else if(tree[i].status == 2) printf("%s, [", "internal node");
