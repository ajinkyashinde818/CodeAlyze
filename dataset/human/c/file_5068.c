...
int height(int id){
    if(T[id].h != -1)                   return T[id].h;
    if(T[id].l == -1 && T[id].r == -1)  T[id].h = 0;
    else if(T[id].l == -1)              T[id].h = height(T[id].r) + 1;
    else if(T[id].r == -1)              T[id].h = height(T[id].l) + 1;
    else                                T[id].h = max( height(T[id].l), height(T[id].r) ) + 1;
    return T[id].h;
}
...
int main(void){
  ...
  for(int i = 0 ; i < n ; i++){
    ...
    T[id].p = T[k].p = z;
    T[k].s = id; T[id].s = k;
    if(id == NIL && k == NIL)       T[z].deg = 0;
    else if(id != NIL && k != NIL)  T[z].deg = 2;
    else                            T[z].deg = 1;
