...
void print(int u){
 ...
 if(T[u].p==-1/*ここを埋める (fill this blank)*/) printf("root, ");
 else if(T[u].l==NIL/*ここを埋める(fill this blank)*/) printf("leaf, ");
...
void calcDepth(int u, int p){
 D[u] = p;
 if(T[u].r!=NIL/*ここを埋める(fill this blank)*/) calcDepth(T[u].r,p/*ここを埋める(fill this blank)*/);
 if(T[u].l!=NIL/*ここを埋める(fill this blank)*/) calcDepth(T[u].l,p+1/*ここを埋める(fill this blank)*/);
}
...
int main(){
 ...
 for(i=0; i<n; i++){
   scanf("%d %d", &id, &k);
   for(j = 0, sib = NIL; j<k; j++){ //子を入力する (input children)
     scanf("%d", &child);
     T[child].p = id;
