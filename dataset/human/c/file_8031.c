...
#define the_number_of_edge 25114123
int edge_cnt[the_number_of_edge];
ll* graph[the_number_of_edge];
ll ans[the_number_of_edge];
bool is_conf[the_number_of_edge];
...
typedef struct{  int from,to;  ll cost;  }graph_heap;
// graph_heap heap[2*MAX_N];
graph_heap heap[ the_number_of_edge ];
...
void dijkstra(ll start){
    rep(i,0,the_number_of_edge){  ans[i] = INF;  is_conf[i] = false;  }
    ans[start] = 0;
    is_conf[start]  = true;
    int size=0;
    rep(i,0,edge_cnt[start]){
        heap[size].from = start;  heap[size].to = graph[start][i*2];
        ans[ heap[size].to ] = graph[start][i*2+1];
        graph_pushHeap( graph[start][i*2+1] , size++ , 0 );
    }
    ll piv;
    while(size){
        if( is_conf[ heap[0].to ] ){  graph_popHeap(size--,0);  continue;  }
        ...
        rep(i,0,edge_cnt[piv]){
            if( ans[ graph[ heap[0].to ][i*2] ] > ans[ heap[0].to ] + graph[ heap[0].to ][i*2+1] ){
                heap[size].from = heap[0].to;   heap[size].to = graph[ heap[0].to ][i*2];
        ...
        rep(j,0,w){
            ...
            rep(k,-2,3){
                rep(l,-2,3){
                    ...
                    ll y=i+k;
                    ll x=j+l;
                    if(is_inArr2(y,x,h,w)==0) continue;
                    if(s[y][x]=='#'){
                        // pushGraph(i*w+j,y*w+x,1,1);
                        continue;
                    }else{
                        if(llabs(k)+llabs(l)<2){
                            pushGraph(i*w+j,y*w+x,0,1);
                        }else{
                            pushGraph(i*w+j,y*w+x,1,1);
                        }
                    }
