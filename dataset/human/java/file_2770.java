/**
 * Created at 20:53 on 2019-06-08
 */
...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    ArrayList<Character> tp = new ArrayList();

    for (int i=0; i<s.length(); i++) {
      sp.add(s.charAt(i));
    }
    Collections.sort(sp, (o1, o2) -> o1-o2);

    for (int i=0; i<t.length(); i++) {
      tp.add(t.charAt(i));
    }
    Collections.sort(tp, (o1, o2) -> o2-o1);

    int len = Math.min(s.length(), t.length());
    for (int i=0; i<len; i++) {
      if (sp.get(i) < tp.get(i)) {
        System.out.println("Yes");
        System.exit(0);
      } else if (sp.get(i) == tp.get(i)) {
        continue;
      } else {
