...
public class Main {
    public static long exponentOfFactorial(long n, long primeNumber) {
        if (n < 0 || primeNumber < 2) {
        ...
        return n / primeNumber
               + exponentOfFactorial(n / primeNumber, primeNumber);
    }
    public static long exponentOfDoubleFactorial(long n, long primeNumber) {
        if (n < 0 || primeNumber < 2) {
            throw new IllegalArgumentException();
        }
        if (n % 2 == 0) {
            long result = exponentOfFactorial(n / 2, primeNumber);
            if (primeNumber == 2) {
                result += n / 2;
            }
            return result;
        }
        return exponentOfFactorial(n, primeNumber)
               - exponentOfDoubleFactorial(n - 1, primeNumber);
    }
    static void solve(MyScanner in, MyWriter out) {
        long n = in.nextLong();
        long ans = Math.min(exponentOfDoubleFactorial(n, 2),
                            exponentOfDoubleFactorial(n, 5));
        out.println(ans);
    }
