...
object Main {
  ...
  def solve(): Unit = {
    ...
    val D = sc.nextInt()
    val G = sc.nextInt() / 100
    val P, C = Array.ofDim[Int](D)
    rep(D) { d =>
      P(d) = sc.nextInt
      C(d) = sc.nextInt / 100
    }
    val INF = 1001
    val dp = Array.fill[Int](D + 1, 110001)(INF)
    dp(0)(0) = 0
    rep(110001) { i =>
      rep(D) { d =>
        val p = P(d)
        ...
        rep(p + 1) { j =>
          if (j < p) {
            if (i - (d + 1) * j >= 0) dp(d + 1)(i) = min(dp(d + 1)(i), dp(d)(i - (d + 1) * j) + j)
          }
          else {
            if (i - (d + 1) * j - c >= 0) dp(d + 1)(i) = min(dp(d + 1)(i), dp(d)(i - (d + 1) * j - c) + j)
          }
        }
      }
    }
    ...
    var ans = INF
    (G until 110001) foreach (i => if (dp(D)(i) != INF) ans = min(ans, dp(D)(i)))
    out.println(ans)
  }
