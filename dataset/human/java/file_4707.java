...
public class Main {
  ...
  static void solve(InputStream is, PrintStream os) {
    ...
    /* read */
    String c = new StringBuilder(sc.next()).reverse().append(0).toString();

    int[][] dp = new int[c
        .length()][2]; // dp[i][j] == i th digit is paid (j == 0), not paid (j == 1)
    ...
    for (int i = 1; i < c.length(); i++) {
      d = c.charAt(i) - '0';
      if (d == 0) {
        dp[i][0] = Math.min(dp[i - 1][0], dp[i - 1][1] + 1 + d);
        dp[i][1] = dp[i - 1][1] + 10 - (1 + d);
      } else if (d < 9) {
        dp[i][0] = Math.min(dp[i - 1][0] + d, dp[i - 1][1] + 1 + d);
        dp[i][1] = Math.min(dp[i - 1][0] + 10 - d, dp[i - 1][1] + 10 - (1 + d));
      } else {
        dp[i][0] = dp[i - 1][0] + d;
        dp[i][1] = Math.min(dp[i - 1][0] + 10 - d, dp[i - 1][1]);
      }
    }
    os.println(dp[c.length() - 1][0]);
  }
