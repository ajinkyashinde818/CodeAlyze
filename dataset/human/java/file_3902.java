...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int Calc()
		{
			...
			var M = 0;
			for (var i = 0; i < N; i++) if (S[i] != 'x') { dict.Add(M++, i); sb.Append(S[i]); }
			var T = sb.ToString();
			if (!IsPalindrome(T)) return -1;
			var s = 0;
			dict[-1] = -1;
			dict[M] = N;
			for (int l = 0, r = M - 1; l <= r; l++,r--) s += Math.Abs((dict[l] - dict[l - 1] - 1) - (dict[r + 1] - dict[r] - 1));
			return s;
		}
...
class SkewHeap<T> : IEnumerable<T>
{
	...
	public SkewHeap(Comparison<T> c) { comp = c; }
...
class BucketList<T> : ICollection<T>, IEnumerable<T>, ICollection, IEnumerable
{
	...
	void SetVariable(string[] command, ref BucketNode<T> x, Func<string, T> parser, Func<T> rand)
	{
		...
		else if (command[1].StartsWith("f")) { if (command.Length > 2) x = LowerBound(parser(command[2])).Item1; else x = LowerBound(rand()).Item1; }
	}
}
	...
	public int[] ShortestPathLengthFrom(int from, Func<E, int> cost)
	{
		...
		while (update)
		{
			...
			foreach (var e in edges)
			{
				...
				if (d[e.From] < Func.Inf && d[e.To] > tmp) { d[e.To] = tmp; update = true; }
			}
		}
		return d;
	}
...
class AVLTree<T> : IEnumerable<T>, ICollection<T>, ICollection, IEnumerable
{
	...
	public static void CheckAVL(Random rand, int N)
	{
		...
		Console.WriteLine(insertErrors && deleteErrors ? "○\t挿入, 削除操作が正しく行われています."
										: insertErrors ? "×\t挿入(または削除)操作に問題があります."
													   : "×\t削除(または挿入)操作に問題があります.");
		Console.WriteLine(avl.IsBalanced() ? "○\tAVL木は平衡条件を保っています."
										   : "×\tAVL木の平衡条件が破れています.");
		Console.WriteLine(avl.IsValidBinarySearchTree() ? "○\tAVL木は二分探索木になっています."
														: "×\tAVL木は二分探索木になっていません.");
...
static class Func
{
	...
	public static void Add<T>(this Stack<T> q, T x) => q.Push(x);
	public static void Add<T>(this Queue<T> q, T x) => q.Enqueue(x);
	public static void Add<T>(this PriorityQueue<T> q, T x) => q.Enqueue(x);
	public static void Add<T>(this Deque<T> q, T x) => q.PushBack(x);
	/// <summary>
	...
	{
		if (i == n) func(array);
		else for (var x = 0; x < n; x++) if (unused[x])
				{
					array[i] = x;
					unused[x] = false;
	...
	public static List<int> GetPrimes(int n)
	{
		...
		for (var p = 0; p < m; p++) if (!composites[p])
			{
				var pnum = 2 * p + 3;
				primes.Add(pnum);
	...
	/// if there is no answer, returns null
	/// </summary>
	...
	public static long ChineseRemainder(params Tuple<long, long>[] modRemainder)
	{
		...
		if (modRemainder.Length == 1) return modRemainder[0].Item2;
		if (modRemainder.Length == 2) return ChineseRemainder(modRemainder[0], modRemainder[1]);
		var tuple = new Tuple<long, long>(1, 0);
		for (var i = 0; i < modRemainder.Length; i++) tuple = new Tuple<long, long>(tuple.Item1 * modRemainder[i].Item1, ChineseRemainder(tuple, modRemainder[i]));
		return tuple.Item2;
	}
	...
	class MultiSorter
	{
		...
		private void SwapRegion(int p, int q, int n) { while (n-- > 0) { var s = a[p]; a[p++] = a[q]; a[q++] = s; } }
