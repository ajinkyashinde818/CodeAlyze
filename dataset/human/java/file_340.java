...
public class Main {
	private static void solve() {
		...
		for (int i = 0; i < n; i++) {
			long x1 = i == n - 2 ? sx : i == n - 1 ? ex : xyr[i][0];
			long y1 = i == n - 2 ? sy : i == n - 1 ? ey : xyr[i][1];
			long r1 = i >= n - 2 ? 0 : xyr[i][2];
			for (int j = 0; j < i; j++) {
				long x2 = j == n - 2 ? sx : j == n - 1 ? ex : xyr[j][0];
				long y2 = j == n - 2 ? sy : j == n - 1 ? ey : xyr[j][1];
				long r2 = j >= n - 2 ? 0 : xyr[j][2];
		...
		double[][] res = Graph.dijkstraD(n, n - 2, i -> n - 1, (i, f) -> {
			for (int j = 0; j < n; j++) {
				if (i == j)
					continue;
				f.f(j, dist[i][j]);
