...
class Solver {
    ...
    void solve(int tc) throws Exception {
        ...
        int N = hp.nextInt(), M = hp.nextInt(), X = hp.nextInt();
        int[][] A = new int[N][];
        for (i = 0; i < N; ++i) A[i] = hp.getIntArray(M + 1);

        int ans = Integer.MAX_VALUE;
        for (i = 0; i < 1 << N; ++i) {
            int[] knowledge = new int[M];
            int cost = 0;
            for (j = 0; j < N; ++j) if ((i >> j) % 2 == 1) {
                cost += A[j][0];
                for (k = 1; k <= M; ++k) knowledge[k - 1] += A[j][k];
            }
            if (hp.min(knowledge) >= X) ans = Math.min(ans, cost);
        }
        hp.println(ans >= Integer.MAX_VALUE >> 1 ? -1 : ans);
    }
}
