...
public class Main {
    ...
    public static void solve(){
        ...
        String s = sc.next();
        int n = s.length();
        char[] nor = s.toCharArray();
        char[] rev = new char[n];
        for (int i = 0; i < n; i++) {
            rev[rev.length - i - 1] = nor[i];
        }
        int nAns=0,rAns=0;
        int norI = 0, revI = 0;
        while ((norI+nAns) < (n + 1+nAns+rAns) / 2 && (revI+rAns) < (n + 1+nAns+rAns) / 2) {
            runWhenEA(func("norI", norI, "revI", revI));
            if(nor[norI]==rev[revI]){
                norI++;
                revI++;
                continue;
            }
