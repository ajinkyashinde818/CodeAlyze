...
public class Main {
	...
	public static void main(String[] args) {
		...
		StringBuilder sb = new StringBuilder(String.copyValueOf(t));
		sb.reverse();
		String tt = sb.toString();

		String[] array = { String.copyValueOf(s), tt };
		Arrays.parallelSort(array);

		if (String.copyValueOf(s).contentEquals(tt)) {
			out(N);
			return;
		}

		if (String.copyValueOf(s).contentEquals(array[0])) {
			out(Y);
		} else {
			out(N);
		}
