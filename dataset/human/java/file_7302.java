...
public class Main {
	...
	static char[] S;
	static char[] s1 = "dream".toCharArray();
	static char[] s2 = "dreamer".toCharArray();
	static char[] s3 = "erase".toCharArray();
	static char[] s4 = "eraser".toCharArray();
	...
	static void check(int from){
		if(from == S.length)flag =  true;
		for(char[] s : list){
			int N = s.length;
			if(from + N > S.length)continue;
			boolean isOK = true;
