...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		int xs, ys, xt, yt, N;
		Tuple<int, int, int>[] bs;
		void Read()
		{
			sc.Read(out xs, out ys, out xt, out yt);
			sc.Read(out N);
			bs = sc.ReadManyLines<int, int, int>(N);
		}
		List<Tuple<int, double>>[] es;
		int V;
		void SolveOne()
		{
			V = N + 2;
			es = new List<Tuple<int, double>>[V];
			for (var i = 0; i < V; i++) es[i] = new List<Tuple<int, double>>();
			var ball = new int[3, V];
			for (var i = 0; i < N; i++)
			{
				ball[0, i] = bs[i].Item1;
				ball[1, i] = bs[i].Item2;
			...
			for (var i = 0; i < V; i++)
			{
				for (var j = i + 1; j < V; j++)
				{
					var dx = ball[0, i] - ball[0, j];
					var dy = ball[1, i] - ball[1, j];
					var r = ball[2, i] + ball[2, j];
					var R = Sqrt(Pow(dx, 2) + Pow(dy, 2));
					var dr = Max(0, R - r);
					AddEdge(i, j, dr);
				}
			}
			var d = DijkstraFrom(N);
			pr.WriteLine(d[N + 1]);
		}
		void AddEdge(int i, int j, double d)
		{
			es[i].Add(new Tuple<int, double>(j, d));
			es[j].Add(new Tuple<int, double>(i, d));
		}
		public double[] DijkstraFrom(int from)
		{
			var d = Enumerable.Repeat(double.PositiveInfinity, V).ToArray();
			...
			while (!queue.IsEmpty)
			{
				...
				foreach (var e in es[v])
				{
					var tmp = d[v] + e.Item2;
					var u = e.Item1;
					if (d[u] > tmp) queue.Enqueue(new Pair<int, double>(u, d[u] = tmp));
				}
			}
			return d;
		}
