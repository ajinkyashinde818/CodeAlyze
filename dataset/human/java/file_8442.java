  ...
  private boolean accept(int k) {
    ...
    for (int a : as) {
      if (a <= prevA) {
        ...
        while (a > 0 && !indexQueue.isEmpty() && indexQueue.peekLast() == a && countQueue.peekLast() >= k) {
          indexQueue.pollLast();
          countQueue.pollLast();
          a--;
        }
        ...
        if (!indexQueue.isEmpty() && indexQueue.peekLast() == a) {
          countQueue.addLast(countQueue.pollLast() + 1);
        } else {
