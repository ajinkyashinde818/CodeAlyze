...
public class Main {
	...
	public void exec() throws Exception{
		...
		final int[] cost = new int[N.length()+1];
		final int[] pay = new int[N.length()+1];
		final int[] ret = new int[N.length()+1];

		for(int i=0;i<N.length();i++) {
			cost[i+1] = N.charAt(i)-'0';
		}
		...
		pay[0] = up;
//		err.println(Arrays.toString(pay));
...
//		err.println(Arrays.toString(ret));
