...
class Main(out: java.io.PrintWriter, sc: Main.InputReader) {
  ...
  def solve(): Unit = {
    ...
    type G = Array[ArrayBuffer[Int]]
    val MAX = 50

    def buildGraph(bit: Long): G = {
      val g = Array.fill[ArrayBuffer[Int]](MAX + 1)(ArrayBuffer())
      REP(MAX + 1) { x =>
        REP(x, 1) { m =>
          if ((bit & (1L << m)) > 0) {
            val r = x % m
    ...
    def reachable(g: G, s: Int, t: Int): Boolean = {
      s == t || { g(s).exists(reachable(g, _, t)) }
    }

    def test(bit: Long): Boolean = {
      val g = buildGraph(bit)
      val res = 0 until N forall { i => reachable(g, A(i), B(i)) }
      debug(s"${java.lang.Long.toBinaryString(bit)} $res")
      res
    }
