...
public class Main
{
    ...
    public static void main(String[] args)
    {
        ...
        long G = sc.nextLong();
        long minRes = 0;
        long[] kosus = new long[D + 1];
        long[] bonus = new long[D + 1];
        for (int i = 1; i < D + 1; i++)
        {
            kosus[i] = sc.nextLong();
            bonus[i] = sc.nextLong();
        ...
        for (int m = 0; m < (1 << D); m++)
        {
            ...
            for (int i = 0; i < D; i++)
            {
                ...
                {
                    //全部ふるの場合は考慮しなくて良い
                    long need = (long) Math.ceil((G - sum) * 1.0 / ((i + 1) * 100));
                    if (need > kosus[i+1] || need < 0) continue;
                    minRes = Math.min(minRes, cou + need);
                }
            }
        }
