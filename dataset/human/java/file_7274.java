...
fun solve(){
    val (k, n) = nextLongList()
    val a = nextLongList()
    val list:MutableList<Long> = mutableListOf()
    for (i in 0 until n - 1) {
        list.add(a[i.toInt() + 1] - a[i.toInt()])
    }
    list.add(k - a[n.toInt() - 1] + a[0])
    list.sortDescending()
    println(list.drop(1).sum())
...
data class Node(val id: Int, var past: Int = -1, var edges: MutableList<Edge> = mutableListOf())
