...
class Solver {
  ...
  public long solve() {
    ...
    long expectation = 0;
    for (int i = 1; i < n; i++) {
      long diff = xs[i] - xs[i - 1];
      inverseSum = mc.add(inverseSum, mc.inverse(i));
      expectation = mc.add(expectation, mc.mul(diff, inverseSum));
    }
    return mc.mul(expectation, mc.getF(n - 1));
  }
}
