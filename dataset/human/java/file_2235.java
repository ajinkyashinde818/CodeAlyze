...
fun solve() {
    val n = nextLong()
    val res = erathos(Math.sqrt(n.toDouble()).toLong())
    val primes = longList()
    for (i in res.indices) {
        if(i >= 2L && i.toLong() == res[i]) primes.add(i.toLong())
    }

    var now = n
    val divs = map<Long, Long>()
    for (i in primes.indices) {
        val d = primes[i]
        while (now > 1L && now % d == 0L) {
            if(divs.contains(d)) divs[d] = divs[d]!! + 1
            else divs[d] = 1
            now /= d
        }
    }
    if(now > 1L) {
        if(divs.contains(now)) divs[now] = divs[now]!! + 1
        else divs[now] = 1
    }

    val tmp = longAry(64)
    for (i in tmp.indices) tmp[i] = (i + 1).toLong()
    val count = tmp.cumsum(addFunc)
    var ans = 0L
