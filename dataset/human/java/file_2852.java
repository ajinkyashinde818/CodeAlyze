...
fun main(args: Array<String>) {
	val n = readInt()
	val xList = readListOfLong()

	val factorials = Array(n+1) { 0L }.apply {
		this[0] = 1L
		this[1] = 1L
	}
	val inverse = Array(n+1) { 0L }.apply {
		this[0] = 1L
		this[1] = 1L
	}
	...
	for(i in 0 until n-1) {
		distances[i] = xList[i+1] - xList[i]
	}

	var probabirity = 0L
	var inverseSum = 0L
	for(i in 0 until n-1) {
		inverseSum += inverse[i+1]
		inverseSum %= MOD
		probabirity += distances[i] * inverseSum % MOD
		probabirity %= MOD
	}

	println(probabirity * factorials[n-1] % MOD)
