...
fun main(args: Array<String>) {
	...
	val frontChars = CharArray(q) { '.' }
	val backChars = CharArray(q) { '.' }
	var frontIdx = q - 1 
	var backIdx = 0
	for(i in 0 until q) {
		val query = readListOfString()
		when(query[0]) {
			"1" -> {
				isReversed = !isReversed
			}
			else -> {
				val (f, c) = query.slice(1 .. 2).map { it[0] }
				if((!isReversed && f == '1') || (isReversed && f == '2')) {
					frontChars[frontIdx] = c
					frontIdx--
				} else {
					backChars[backIdx] = c
					backIdx++
	...
	val ans = frontChars.filter { it != '.' }.joinToString("") + s + backChars.filter { it != '.' }.joinToString("")


	println(if(isReversed) ans.reversed() else ans)
	pw.flush()
}
