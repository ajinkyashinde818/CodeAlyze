...
namespace Tmp
{
	...
	class Problem : IDisposable
	{
		...
		{
			sc.Read(out N, out M);
			A = sc.Board(N, N, (c, x, y) => c == '#');
			B = sc.Board(M, M, (c, x, y) => c == '#');
		}
		void SolveOne()
		{
			for (var i = 0; i + M <= N; i++)
				...
				{
					var flag = true;
					for (var x = 0; flag && x < M; x++)
						for (var y = 0; flag && y < M; y++)
							flag &= A[i + x, j + y] == B[x, y];
					if (flag) { pr.WriteLine("Yes"); return; }
				}
			pr.WriteLine("No");
		}
...
class Treap<T>
{
	...
	class Node
	{
		...
		public static int Size(Node t) => t == null ? 0 : t.size;
