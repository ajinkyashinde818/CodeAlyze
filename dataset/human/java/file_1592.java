...
fun match(aImg: Array<String>, bImg: Array<String>, 
			sx: Int, sy: Int, m: Int): Boolean {
	var isMatched = true
	for(i in 0 until m) {
		val partialAImgRow = aImg[i+sy].slice(sx until sx + m)
		if(partialAImgRow != bImg[i]) {
			isMatched = false
		}
	}
	return isMatched
...
fun main(args: Array<String>) {
	...
	for(i in 0 until n-m+1) {
		for(j in 0 until n-m+1) {
			isMatched = isMatched || match(aImg, bImg, i, j, m)	
		}
	}
	println(if(isMatched) "Yes" else "No")
	pw.flush()
}
