...
fun main(args: Array<String>) {
    ...
    var cur = 0
    var history = mutableListOf<Int>(cur)
    ...
    val removed = history.slice(offset until history.size)
    val loopSize = removed.size.toLong()
    val firstStep = (k - offset.toLong() < 0L)
    val ans = (if(firstStep) history[k.toInt()] else removed[((k-offset.toLong()) % loopSize).toInt()]) + 1
    println(ans)
	pw.flush()
}
