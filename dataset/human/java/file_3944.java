...
public class Main {
	...
	public static void main(String[] args) {
		...
		List<Integer> Bs=sc.nextArrayIntsN(N);
		List<Integer> Cs=sc.nextArrayIntsN(N-1);
		
		int ans=Bs.get(As.get(0)-1);
		for(int i=1;i<N;i++) {
			
			if(As.get(i)==As.get(i-1)+1) {
				ans+=Bs.get(As.get(i)-1)+Cs.get(As.get(i-1)-1);;
			}else {
				ans+=Bs.get(As.get(i)-1);
			}
