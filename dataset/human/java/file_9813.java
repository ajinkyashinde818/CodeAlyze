...
public class Main {
	...
	public static void check(long arr[][], long x, long val[], long cost, int idx) {
	...
	//		System.out.print(val[i]+" ");
		...
		if(idx>=arr.length)
			return ;
		long arx[]=new long[val.length];
		for (int i = 0; i < val.length; i++){
			arx[i] =val[i]+ arr[idx][i + 1];
		//	System.out.print(arx[i]+" ");
			
		}
		//System.out.println();
		check(arr, x, arx, cost + arr[idx][0], idx + 1);
//		for (int i = 0; i < val.length; i++)
//			val[i] -= arr[idx][i + 1];
		check(arr, x, val, cost, idx + 1);
	}
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		long x = sc.nextLong();
		min=Long.MAX_VALUE;
		long arr[][] = new long[(int) n][(int) m + 1];
		for (int i = 0; i < n; i++) {
			for (int j = 0; j <= m; j++)
				arr[i][j] = sc.nextLong();
		}
