...
public class Main {
    ...
    static void solve(MyScanner in, MyWriter out) {
        String n = in.next();
        n = new StringBuilder(n).reverse().append('0').toString();
        ...
        for (int i = 0; i < n.length(); i++) {
            int c = n.charAt(i) - '0';
            for (int kurisage = 0; kurisage < 2; kurisage++) {
                for (int d = 0; d <= 9; d++) {
                    int r = d - c - (kurisage == 1 ? 1 : 0);
                    int j = 0;
                    if (r < 0) {
                        j = 1;
                        r += 10;
                    }
                    setIfMin(dp[i + 1], j, dp[i][kurisage] + d + r);
                }
            }
        }
        out.println(dp[n.length()][0]);
    }
