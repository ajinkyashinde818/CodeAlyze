...
public class Main {
	...
	static void solve() {
		P start = new P(ni(), ni(), 0), goal = new P(ni(), ni(), 0);
		n = ni();
		P ps[] = new P[n+2];
		ps[0] = start;
		for(int i=1;i<=n;i++) {
	...
	static double[] dijkstra(int start) {
		...
		fill(dist, dinf);
		dist[start] = 0;
		PriorityQueue<Vertex> pq = new PriorityQueue<>((v1, v2)->v1.dist.compareTo(v2.dist));
		pq.add(new Vertex(start, 0));
		while(!pq.isEmpty()) {
			Vertex v = pq.poll();
			if(v.dist > dist[v.id])continue;
			for(Edge e:graph[v.id]) {
				if(dist[e.to] > dist[v.id] + e.cost) {
					dist[e.to] = dist[v.id] + e.cost;
	...
	static double dist(P p1, P p2) {
		...
		double res = Math.sqrt(dx*dx + dy*dy) - p1.r - p2.r;
		return res <= 0?0:res;
	}
