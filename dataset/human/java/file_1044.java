...
class Solver {
  final ModCalculator mc = new ModCalculator(1_000_000_007L);
  final int n;
  final char[] cs;
  
  public Solver(int n, char[] cs) {
    this.n = n;
    this.cs = cs;
  }
  
  public long solve() {
    if (cs[0] == 'W' || cs[2 * n - 1] == 'W') {
      return 0;
    }
    ...
    int leftCount = 0;
    long answer = 1;
    for (boolean isLeft : isLefts) {
      if (isLeft) {
        leftCount++;
      } else {
        answer = mc.mul(answer, leftCount);
        leftCount--;
      }
    }
    if (leftCount != 0 || answer == 0) {
      return 0;
    }
    return mc.mul(answer, mc.getF(n));
  }
}
