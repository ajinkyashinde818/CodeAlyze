...
//    if(n>3037007383L){
  ...
  static void solve(){
    String s=sc.next();
    List<IC> list=new LinkedList<>();
    for(int i=0;i<s.length();i++){
      list.add(new IC(i,s.charAt(i)));
    }
    int q=sc.nextInt();
    int rev=1;
    for(int i=s.length();i<+s.length()+q;i++){
      if(sc.nextInt()==1){
        rev*=-1;
      }else{
        list.add(new IC(i*(sc.nextInt()*2-3)*rev,sc.next().charAt(0)));
      }
    }
    if(rev==1){
      list.sort((s1,s2)->Integer.compare(s1.i,s2.i));
    }else{
      list.sort((s1,s2)->Integer.compare(s2.i,s1.i));
    }
    for(IC ic:list){
      out.print(ic);
    }
