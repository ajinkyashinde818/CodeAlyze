...
signed main(){
    ...
    VEC(ll,a,n);
    VEC(ll,b,n);
    a.push_back(reduce(all(a),0LL,bit_xor()));
    b.push_back(reduce(all(b),0LL,bit_xor()));
    auto a2=a,b2=b;
    Sort(a2);
    Sort(b2);
    if(a2!=b2)return out(-1);
    ll ans=0;
    rep(n)ans+=a[i]!=b[i];
    unordered_map<ll,ll>press;
    each(i,a){
        if(press.count(i))i=press[i];
        else{
            const ll a=press.size();
            i=press[i]=a;
        }
    }
    each(i,b){
    ...
    rep(n+1)uf.unite(a[i],b[i]);
    rep(press.size())if(uf[i]==i&&uf.size(i)>1&&!uf.find(i,a[n]))ans++;
    out(ans);
}
