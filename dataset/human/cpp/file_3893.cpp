...
//Dijkstra法　は　隣接リスト　で　O(V^2) ,PriorityQueue で O(ElogV)
//密グラフなら隣接リストでok
...
void solve() {
    ...
    dist[s][t] = dist[t][s] = abs(sp - tp);
    rep(i, n) {
        rep(j, n) {
            double di = abs(c[i] - c[j]);
            dist[i][j] = dist[j][i] = max(0.0, di - c[i].r - c[j].r);
        }
    }
    ...
    while(true) {
        int v = -1;
        rep(u, n + 2) {
            if(!seen[u] && (v == -1 || d[u] < d[v])) v = u;
        }
        if(v == -1) break;
        seen[v] = true;
        rep(u, n + 2) {
            chmin(d[u], d[v] + dist[v][u]);
        }
    }
    cout << d[t] << endl;
}
