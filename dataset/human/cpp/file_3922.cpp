...
int main() {
    ...
    V<double> d(N + 2, INF);
    priority_queue<P<double, ll>, vector<P<double, ll>>, greater<P<double, ll>>> que;
    ...
    while (!que.empty()) {
        ...
        NREP(sv, N + 1) {
            ...
            double pd = sqrt(sq(x[sv] - x[v]) + sq(y[sv] - y[v])) - r[sv] - r[v];
            if (pd < 0) pd = 0;
            que.push(mp(dis + pd, sv));
        }
    }
