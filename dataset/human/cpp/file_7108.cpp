...
int main() {
	...
	for (auto [k, v] : mp)if (cnt[k].size())que.push({ v,k });
	vector<P> p;
	for (auto [k, v] : mp) if (cnt[k].size() != v)p.push_back({ k,v - cnt[k].size() });
	Array ans(n);
	while (que.size()) {
		auto x = que.top();
		que.pop();
		...
		mp[x.second]--;
		if (p.back().first == x.second)swap(p.back(), p[p.size() - 2]);
		ans[cnt[x.second].back()] = p.back().first;
		mp[p.back().first]--;
		cnt[x.second].pop_back();
		p.back().second--;
		if (p.back().second == 0)p.pop_back();
		if (cnt[x.second].size() != 0)que.push(x);
	}
	cout << "Yes" << "\n";
	REP(i, n)cout << ans[i] << " ";
	cout << endl;
