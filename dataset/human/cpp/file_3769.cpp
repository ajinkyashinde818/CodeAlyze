...
ll n=1002;
vv(double,d,n,n,DINF);
vec(double,cost,n,DINF);
void dijkstra(ll at){
    vec(bool,used,n);
    cost[at]=0;
    ...
    while(!q.empty()){
        ...
        if(used[at])continue;
        used[at]=1;
        rep(n)if(update_min(cost[i],d[at][i]+cnt)) q.push({cost[i],i});
    }
}
...
signed main(){
    ...
    rep(n){
        d[n][i]=d[i][n]=max(dist(s,a[i].first)-a[i].second,0.0);
        d[n+1][i]=d[i][n+1]=max(dist(g,a[i].first)-a[i].second,0.0);
    }
    rep(n-1)rep(j,i+1,n)d[i][j]=d[j][i]=max(dist(a[j].first,a[i].first)-a[i].second-a[j].second,0.0);
    n+=2;
    dijkstra(n-2);
    out(cost[n-1]);
}
