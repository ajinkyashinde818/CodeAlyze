...
void solve(){
	...
	gp_hash_table<int,pair<int,int>,custom_hash> pos, posb;
	...
	for(auto e: posb){
		...
		auto rang2 = pos[e.first];



		int lo = max(rang2.first - rang.second, 0);
		int hi = min(rang2.second - rang.first, n - 1);

		if (lo <= hi){
			rot[lo]++;
			rot[hi+1]--;
		}
		// if (rang.first <= rang2.first){
		// 	int k = rang2.first - rang.first;
		// 	int l = max(0, rang2.first - rang.second);
		// 	trace(l, k);
		// 	if (l <= k){
		// 		rot[l]++;
		// 		rot[k + 1]--;
		...
		lo = max(rang2.first - rang.second, 0);
		hi = min(rang2.second - rang.first, n - 1);
	...
	for(int i = 0; i < n; ++i){
		if (rot[i] == 0){
			if (i != 0)
				rotate(b.begin(), b.begin() + (n - i), b.end());
			break;
		}
	}
