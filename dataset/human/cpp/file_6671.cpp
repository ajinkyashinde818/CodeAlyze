...
bool intersect(const circle& c, const segment& s) {
	return dist(s, c.c) < c.r + EPS && (dist(s.a, c.c) > c.r || dist(s.b, c.c) > c.r);
}
...
int main() {
	...
	for(int n; cin >> n, n;) {
		...
		for(int i = 0; i < n; ++i) {
			int wx, wy, r;
			cin >> wx >> wy >> r;
			walls.push_back(circle(point(wx, wy), r));
		}
		...
		for(int i = 0; i < m; ++i) {
			int tx, ty, sx, sy;
			cin >> tx >> ty >> sx >> sy;
			const segment seg(point(tx, ty), point(sx, sy));
			string result = "Danger";
			for(int j = 0; j < n; ++j) {
				if(intersect(walls[j], seg)) {
					result = "Safe";
					break;
				}
			}
