...
int main() {
	...
	SINT64 N;
	cin >> N;
	vector<vector<double>> dat(N,vector<double>(N));

	vector<double> sdata(N);
	vector<double> gdata(N);
	...
	rep(i,0,N) {
		rep(j,i+1,N) {
			double buf1 = ABS(data[i].S.F - data[j].S.F);
			double buf2 = ABS(data[i].S.S - data[j].S.S);
			double buf = sqrt(buf1*buf1+buf2*buf2);
			dat[i][j] = MAX(0,buf-data[i].F-data[j].F);
			dat[j][i] = MAX(0,buf-data[i].F-data[j].F);
		}
	}


	rep(i,0,N) {
		double buf1 = ABS(data[i].S.F - xs);
		double buf2 = ABS(data[i].S.S - ys);
		double buf = sqrt(buf1*buf1+buf2*buf2);
		sdata[i] = MAX(0,buf-data[i].F);
	}
	...
	rep(i,0,N) {
		rep(j,0,N) {
			if (i != j) {
				if (sdata[j] > sdata[i]+dat[i][j]) {
					sdata[j] = sdata[i]+dat[i][j];
					q.push(j);
				}
			}
		}
	...
	while (q.size() != 0) {
		...
		rep(i,0,N) {
			if (i != buf) {
				if (sdata[i] > sdata[buf]+dat[buf][i]) {
					sdata[i] = sdata[buf]+dat[buf][i];
					q.push(i);
				}
			}
		}
	...
	rep(i,0,N) {
		double buf1 = ABS(data[i].S.F - xg);
		double buf2 = ABS(data[i].S.S - yg);
		double buf = sqrtl(buf1*buf1+buf2*buf2);

		ans = MIN(ans,sdata[i]+MAX(0,buf-data[i].F));
	}

	double buf1 = ABS(xs - xg);
	double buf2 = ABS(ys - yg);
	double buf = sqrtl(buf1*buf1+buf2*buf2);
	ans = MIN(ans,buf);
