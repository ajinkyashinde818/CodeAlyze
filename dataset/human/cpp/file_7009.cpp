...
map<ll,ll> mp,mp2;
ll ng=0;
ll swp(ll ind1,ll ind2){
  if(ind1>=n||ind1<0||ind2>=n||ind2<0){
    ng++;
    return 0;
  }
  ll dummy=b[ind1];
...
int main(){
  ...
  if(yabanum==-1){
    ...
    ll edi2=-1;
    for(ll i=0;i<n;i++){
      if(yabanum==a[i]||yabanum==b[n-1-i]){
        if(sti==-1){
          sti=i;
        }
        edi=i;
    ...
    for(ll i=0;i<kosu;i++){
      if(i<sti){
        swp(n-1-i,n-1-(sti2+ireta));
        //cout<<n-1-i<<sti2+ireta<<endl;
      }else{
        swp(n-1-(edi+1+i-sti),n-1-(sti2+ireta));
        //cout<<n-1-(edi+1+i-sti)<<sti2+ireta<<endl;
      }
      ireta++;
    }
    if(ng==1){
