...
int main() {
  ...
  // source -> sink
  g[source].push_back({sink, distance(S, G)});

  // source -> cosmic
  for (int i = 0; i < n; i++) {
    double cost = 0.0;
    // 外側にあるなら
    if (distance(S, cs[i].p) > cs[i].r) {
      cost = distance(S, cs[i].p) - cs[i].r;
    }

    g[source].push_back({i, cost});
  }
  ...
  for (int i = 0; i < n; i++) {
    double cost = 0.0;
    // 外側にあるなら
    if (distance(G, cs[i].p) > cs[i].r) {
      cost = distance(G, cs[i].p) - cs[i].r;
    }

    g[i].push_back({sink, cost});
  }
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      double cost = 0.0;
      if (cs[i].r + cs[j].r < distance(cs[i].p, cs[j].p)) {
        cost = distance(cs[i].p, cs[j].p) - cs[i].r - cs[j].r;
      }
