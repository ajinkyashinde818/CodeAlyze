...
int solve(int n, int c, std::vector<int> x, std::vector<int> v) {
	ST st(n, MAX);
	...
	CHMAX(res, st.query(0, n));

	now = 0;
	RREP(i, n) {
		now += v[i];
		CHMAX(res, st.query(0, i) - 2 * (c - x[i]) + now);
	}
	return res;
}
