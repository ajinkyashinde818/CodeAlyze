...
struct Trie {
  ...
  int64 dfs(int idx, int dep) {
    if(nodes[idx].accept.size()) return 0;
    int64 ret = 0;
    for(int i = 0; i < 2; i++) {
      if(nodes[idx].nxt[i] == -1) {
        for(int64 j = 0; j < 62; j++) {
          if(((L - dep) >> j) & 1) {
            ret ^= 1LL << j;
            break;
          }
