...
signed main() {
    ...
    cin >> N >> C;
    ll X[N], V[N + 1];
    ll sum1[N + 1], sum2[N + 1], sum3[N + 1], sum4[N + 1];
    sum1[0] = sum2[0] = sum3[N] = sum4[N] = 0;
    rep(i, N) cin >> X[i] >> V[i + 1];
    V[0] = 0;
    rep(i, N) V[i + 1] += V[i];
    rep(i, N) {
        sum1[i + 1] = V[i + 1] - X[i];
        sum2[i + 1] = V[i + 1] - 2 * X[i];
        chmax(sum1[i + 1], sum1[i]);
        chmax(sum2[i + 1], sum2[i]);
    }
    dec(i, N - 1, 0) {
        sum3[i] = V[N] - V[i] - (C - X[i]);
        sum4[i] = V[N] - V[i] - 2 * (C - X[i]);
        chmax(sum3[i], sum3[i + 1]);
        chmax(sum4[i], sum4[i + 1]);
    }
    ll res = 0;
    rep(i, N + 1) chmax(res, max(sum1[i] + sum4[i], sum2[i] + sum3[i]));
    cout << res << endl;
}
