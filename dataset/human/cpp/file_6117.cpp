...
LL solve( const LL C, const VT< LL > &X, const VT< LL > &V )
{
	const int N = SZ( X );
	...
	partial_sum( ALL( gains ), begin( gains ) );

	VT< LL > maxes( 1 );
	partial_sum( ALL( gains ), BI( maxes ), []( const LL m, const LL v ){ return max( m, v ); } );

	LL res = max( 0LL, maxes[N] ), pos = C, walked = 0, eaten = 0;
	for ( int i = N - 1; 0 <= i; --i )
	{
		eaten += V[i];
		walked += pos - X[i];
		chmax( res, eaten - walked * 2 + maxes[i] );
...
int main()
{
	...
	VT< LL > X_rev, V_rev;
	transform( X.rbegin(), X.rend(), BI( X_rev ), bind( minus< LL >(), C, _1 ) );
	reverse_copy( ALL( V ), BI( V_rev ) );

	cout << max( solve( C, X, V ), solve( C, X_rev, V_rev ) ) << endl;
