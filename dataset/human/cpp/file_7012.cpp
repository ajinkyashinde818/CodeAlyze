...
signed main() {
	...
	std::priority_queue<S> pq;
	{
		std::set<int> set;
		std::map<int, int> A, B;
		REP(i, n) ++A[a[i]], ++B[b[i]], set.insert(a[i]), set.insert(b[i]);
		for (auto& x : set) pq.emplace(S{ A[x], B[x], x });
	}
	V<PAIR> ans;
	while(!pq.empty()) {
		V<S> v;
		...
		while (!pq.empty()) {
			...
			if (v.size() >= 2 && f == 3) {
				if (v[0].x && v.back().y) {
					ans.emplace_back(v[0].v, v.back().v);
					--v[0].x;
					--v.back().y;
					ok = true;
					break;
				}
				else if (v.back().x && v[0].y) {
					ans.emplace_back(v.back().v, v[0].v);
					--v.back().x;
					--v[0].y;
					ok = true;
					break;
		...
		if (!ok) EOUT("No");
		while (!v.empty()) {
			if (v.back().x || v.back().y) pq.emplace(v.back());
			v.pop_back();
		}
	}
	std::sort(ALL(ans));
