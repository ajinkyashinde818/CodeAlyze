...
ll n, xs, ys, xt, yt, x[1002], y[1002], r[1000];
long double d[1002][1002], ans[1002];

long double len(ll i, ll j) {
  return sqrt((long double)((x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j])));
}

long double dijkstra() {
  priority_queue<P, vector<P>, greater<P>> pq;
  for (ll i = 0; i < n+2; i++) ans[i] = DINF;
  ans[n] = 0.0;
  pq.push(P(0.0, n));
  while (!pq.empty()) {
    P p = pq.top(); pq.pop();
    ll from = p.second; long double l = p.first;
    if (ans[from] < l - EPS) continue;
    for (ll i = 0; i < n+2; i++) {
      long double l2 = l + d[from][i];
      if (l2 < ans[i] - EPS) {
        ans[i] = l2;
        pq.push(P(l2, i));
      }
...
int main() {
  ...
  scanf("%lld%lld", &xt, &yt);
  scanf("%lld", &n);
  for (ll i = 0; i < n; i++) scanf("%lld%lld%ld", &x[i], &y[i], &r[i]);
  x[n] = xs, y[n] = ys, x[n+1] = xt, y[n+1] = yt;
  for (ll i = 0; i < n; i++) for (ll j = 0; j < n; j++) d[i][j] = max(0.0L, len(i, j) - r[i] - r[j]);
  for (ll i = 0; i < n; i++) d[i][n] = d[n][i] = max(0.0L, len(i, n) - r[i]);
  for (ll i = 0; i < n; i++) d[i][n+1] = d[n+1][i] = max(0.0L, len(i, n+1) - r[i]);
  d[n][n+1] = d[n+1][n] = len(n, n+1);
  printf("%.15Lf\n", dijkstra());
}
