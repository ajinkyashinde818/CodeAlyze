...
ll dfs(ll d, ll depth) {
  if (depth > l) return 0;
  if (d == -1) {
    ll counter = 1, tmp = l - depth + 1;
    while (tmp % 2 == 0) tmp /= 2, counter *= 2;
    return counter;
  }
  if (tree[d].leaf) return 0;
  return dfs(tree[d].left, depth + 1) xor dfs(tree[d].right, depth + 1);
}
...
int main() {
  ...
  for (ll i = 0; i < 20; i++) {
    ...
    ll x = 0;
    while (st.count(x) == 1) x++;
    printf("%lld\n", x);
    tmp.push_back(x);
    ll counter = 1, tmp = i+1;
    while (tmp % 2 == 0) tmp /= 2, counter *= 2;
    printf("%lld!\n",  counter);
  }
  */
