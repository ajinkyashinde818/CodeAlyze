...
int main(){
    ...
    REP(i,n){
        if(i){
            ccv[i]=max(ccv[i-1], cv[i+1]-x[i]);
            cccv[i]=max(cccv[i-1], cv[i+1]-x[i]*2);
        }else{
            ccv[i]=max(0ll,cv[i+1]-x[i]);
            cccv[i]=max(0ll,cv[i+1]-x[i]*2);
        }
    }
    ...
    REP(i,n){
        ...
        dprint(cv[n]-cv[i]);
        dprint(c-x[i]);
        chmax(ma, cv[n]-cv[i] - (c-x[i]));
        if(i){
            dprint(ccv[i-1],cccv[i-1]);
            chmax(ma, cv[n]-cv[i]-(c-x[i])*2+ccv[i-1]);
            dprint(cv[n]-cv[i]-(c-x[i])*2+ccv[i-1]);
            chmax(ma, cv[n]-cv[i]-(c-x[i])+cccv[i-1]);
            dprint(cv[n]-cv[i]-(c-x[i])+cccv[i-1]);
        }
    }
    cout<<ma<<endl;
