...
public:
	...
	GCD(VLL ls) {
		...
		this->constn = 310;
		this->constn = min(this->constn, (LL)ls.size());

		groop = VLL(this->constn, 1);

		for (i = 0;i < this->constn;i++) {
			this->groop[i] = this->ls[i];
		}

		for (i = 0;i < this->ls.size();i++) {
			j = i % this->constn;

			this->groop[j] = MYCP::GreatestCommonFactor(this->groop[j], this->ls[i]);
		}
	}
	...
	LL ClaGCD(LL index) {
		...
		for (i = 0;i < this->constn;i++) {
			if (i != k) {
				ans = MYCP::GreatestCommonFactor(ans, groop[i]);
			}
		}

		for (i = k;i < this->ls.size();i += this->constn) {
			if (i != index) {
				ans = MYCP::GreatestCommonFactor(ans, this->ls[i]);
			}
		}
...
int main(void) {
	...
	if (MYCP::Count(a, [](LL x) {return x == 0;})) {
		k = 0;
		for (i = 0;i < n;i++) {
			k += abs(a[i]);
		}
