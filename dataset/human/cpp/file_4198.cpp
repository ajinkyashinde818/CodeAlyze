...
int main(){
	vector<C>in{C(pin(),0),C(pin(),0)};
	int n;
	cin>>n;
	rep(i,n)in.pb(CIN());
	n+=2;
	vvd cost(n,vd(n));
	rep(i,n)rep(j,n)cost[i][j]=max(0,abs(in[i].c-in[j].c)-in[i].r-in[j].r);
	vd d(n,10000000000ll);
	d[0]=0;
	vi used(n);
	while(1){
		int v=-1;
		rep(i,n)if(used[i]==0&&(v==-1||d[v]>d[i]))v=i;
		if(v==-1)break;
		used[v]=1;
		rep(i,n)d[i]=min(d[i],d[v]+cost[v][i]);
	}
	cout<<shosu(9)<<d[1]<<endl;
}
