...
bool containsImage(const vs& imageA, const vs& imageB, int topLeftX, int topLeftY) {
    repeat(i, imageB.size()) {
        repeat (j, imageB.size()) {
            if (imageA.size() <= topLeftX + i
                    || imageA.size() <= topLeftY + j) {
                return false;
            }
            if (imageB[i][j] != imageA[topLeftX + i][topLeftY + j]) {
                return false;
            }
        }
    }
...
int main(){
    ...
    repeat(k, N) {
        repeat (l, N) {
            if (containsImage(imageA, imageB, k, l)) {
                cout << "Yes" << endl;
                return 0;
            }
        }
