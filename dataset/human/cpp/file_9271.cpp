...
typedef pair<ll, P> Q;
typedef vector<vector<ll> > R;
typedef complex<double> C;
#define cx real()
#define cy imag()
#define FOR(i,a,b) for(ll i = (a);i<(b);(i)++)
#define _FOR(i,a,b) for(ll i = (b)-1;i>(a)-1;(i)--)
const ll INF = 1LL << 60;
...
ll pw(ll i, ll j) {
	if (j == 0) return 1;
	if (j % 2 == 1) return i * pw(i, j - 1) % mod;
	ll tmp = pw(i, j / 2);
	return tmp * tmp % mod;
}
...
int main(){
	...
	b = vector<vector<char> > (m,vector<char>(m));
	FOR(i,0,n)FOR(j,0,n) cin >> a[i][j];
	FOR(i,0,m)FOR(j,0,m) cin >> b[i][j];

	FOR(ix,0,n-m+1)FOR(iy,0,n-m+1){
		bool flag = true;
		FOR(bx,0,m){
			FOR(by,0,m){
				if(b[bx][by] != a[bx+ix][by+iy]){flag =false;break;}
				if(b[bx][by] != a[bx+ix][by+iy]){flag =false;break;}
			}
			if(!flag) break;
		}
