...
int main() {
  scanf("%lld", &n);
  ll p = INF, m = -INF, sum = 0;
  for (ll i = 0; i < n; i++) scanf("%lld", &a[i]);
  for (ll i = 0; i < n; i++) if (a[i] < 0) m = max(m, a[i]); else p = min(p, a[i]);
  ll counter = 0;
  for (ll i = 0; i < n; i++) if (a[i] < 0) counter++;
  for (ll i = 0; i < n; i++) sum += a[i] < 0 ? -a[i] : a[i];
  ll ans = sum;
  if (counter % 2 != 0) ans -= min(-m, p) * 2;
  printf("%lld\n", ans);
}
