...
int main(){
    ...
    rep(i,0,n){
        ...
        cnt[a[i].first]++;
        a[i].second=i;
        Min(le[a[i].first],(ll)i);
        Max(ri[a[i].first],(ll)i);
    }
    vector<pll> b(n);
    rep(i,0,n){
        cin>>b[i].first;
    ...
    ll ma=0;
    rep(i,1,n+1)Max(ma,cnt[i]);
    if(ma>n){
        cout<<"No"<<endl;
        return 0;
    }
    segtree<pll,op,e> seg(b);
    segtree<pll,op2,e2> seg2(b);
    rep(i,0,n){
        if(a[i].first!=b[i].first)continue;
        ll nl=le[a[i].first],nr=ri[a[i].first];
        bool bo=false;
        if(nl>0){
            pll p=seg.prod(0,nl);
            if(p.first!=a[i].first){
                bo=true;
                ll num=p.second;
                seg.set(num,{b[i].first,num});
                seg.set(i,{p.first,i});
                seg2.set(num,{b[i].first,num});
                seg2.set(i,{p.first,i});
                swap(b[i],b[num]);
            }
            if(bo)continue;
            p=seg2.prod(0,nl);
