...
ll answer = 0;
ll n, m;
using circle = struct {ll x, y, r;};
vector<circle> cs;
vector<ld> dist;

ld distance_between_circles(circle& c1, circle& c2){
    ll x1 = c1.x, x2 = c2.x;
    ll y1 = c1.y, y2 = c2.y;
    ll r1 = c1.r, r2 = c2.r;
    ...
    else return sqrt(d2) - r1 - r2;
}

void dijkstra(ll start, ll n) {
    priority_queue<pair<ld, ll>, vector<pair<ld, ll>>, greater<pair<ld, ll>>> edge_costs;
    dist = vector<ld>(n, ld(INF)); // vertex_pre = vl(n, -1);
    dist[start] = 0;
    edge_costs.push({0, start});

    while (!edge_costs.empty()) {
        pair<ld, ll> edge_cost = edge_costs.top();
        edge_costs.pop();
        ll idx = edge_cost.second;
        circle c1 = cs[idx];
        ld cost = edge_cost.first;
        if (dist[idx] < cost) continue;

        rep(i, cs.size()){
            circle c2 = cs[i];
            ld d = distance_between_circles(c1, c2);
            if (dist[i] > dist[idx] + d){
                dist[i] = dist[idx] + d;
                edge_costs.push({dist[i], i});
            }
        }
    }
}
