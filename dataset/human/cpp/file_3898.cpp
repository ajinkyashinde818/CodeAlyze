...
struct Barrier{
  int x,y;
  int radius;
  Barrier(int a,int b,int r):x(a),y(b),radius(r){
  }
};

double Distance(Barrier a, Barrier b){
  double result=std::abs(std::complex<double>(a.x-b.x,a.y-b.y))-a.radius-b.radius;
...
int main(){
  ...
  int n;
  cin>>n;
  Graph<double> graph(n+2);



  std::vector<Barrier> barrier;
  barrier.push_back(Barrier(xs,ys,0));
  for(int i=0;i<n;i++){
    int x,y,r;
    cin>>x>>y>>r;
    barrier.push_back(Barrier(x,y,r));
  }
  barrier.push_back(Barrier(xt,yt,0));

  for(int i=0;i<n+2;i++){
    for(int j=i+1;j<n+2;j++){
      graph.SetUndirectedEdge(i,j,Distance(barrier[i],barrier[j]));
    }
  }

  std::vector<double> mincost=Dijkstra(graph,0,DBL_MAX/3);
